...
V- 0005 this repo is maintained 
 ...
 Some apps need a slight mod for atomic overwrite to work best in linux. For every app just feed the code to ai- it will audit and tell you how the app works. Some encrypt in memory, some use chunk based streaming to encrypt. Some overwrite the original file, some pake a new file with output. Latest ver of chatgpt made all the code, and for every app here i tested it quite well. Also of note, it is silly to expect security from home made crypto apps or a spyware os such as windows. Some of the apps here are really good, some are easily crackable. Lastly, feel free to use the code however you wish. If you want to make your own repo with all the code and credit yourself- go for it- as it would help promote rust for crypto. Zero restrictions on the code. (Sometimes Ai puts license terms in when making the readme, i do not bother to edit it) 
